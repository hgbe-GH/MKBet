...
L’adresse e-mail est l’identifiant de connexion. L’inscription exige un mot de passe de 10 à 128 caractères et sa confirmation, puis Supabase impose la confirmation de l’adresse avant l’accès privé. L’interface ne propose plus de magic link : le secours passe par `/forgot-password`, le callback Auth et `/auth/update-password`. Le callback n’accepte le changement que lorsque les claims portent l’AMR `recovery`, puis la session de récupération est fermée.
