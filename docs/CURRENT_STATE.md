...
- Le calendrier protégé `/markets/calendar` lit les marchés existants sous RLS, groupe leurs dates opérationnelles par semaine UTC et distingue ouverture, fermeture des mises et échéance du fait. Ses filtres accessibles conservent la semaine, les liens précédent/suivant conservent les filtres, et les horaires affichent explicitement UTC. La page Marchés y donne accès sans ajouter d’onglet mobile. Le ticket expose les libellés Simple/Combiné, conserve l’ordre des jambes et rappelle que le devis PostgreSQL actif, avec corrélation exacte si présente, est la seule source de la cote et du retour affichés; chaque surface de ticket génère ses propres identifiants de mise accessibles.
