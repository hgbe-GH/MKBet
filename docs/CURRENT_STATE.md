...
- Le parcours de récupération `/forgot-password` → callback Auth → `/auth/update-password` vérifie l’AMR `recovery` côté serveur. Le callback accepte les liens Supabase PKCE avec `code` comme les liens de récupération avec `token_hash` ; après modification, il contrôle la fermeture locale de la session puis redirige vers la confirmation publique sûre `/login?notice=password-updated` ; un cleanup retourné en erreur ou levé reste générique et n’annonce jamais de succès.
...
- Lorsqu’un marché persistant `OPEN` dépasse `closes_at`, le détail le présente comme « Mises fermées » et désactive ses issues : cette protection ergonomique complète, sans remplacer, le contrôle autoritaire du devis PostgreSQL.
...
- `pnpm test` : 292 tests réussis dans 48 fichiers.
